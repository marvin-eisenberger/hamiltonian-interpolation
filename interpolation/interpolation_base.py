...
class InterpolationModuleMultiscale(InterpolationModule, InterpolationModuleMultiscaleBase):
    """Base class for interpolation methods based on a local distortion potential"""
...
class InterpolationModuleSingle(InterpolationModule):
    ...
    def __init__(self, energy: InterpolationEnergy, shape_array, interpolation_coeff):
        ...
        assert all([coeff >= 0 for coeff in interpolation_coeff]) and sum(interpolation_coeff) == 1, \
    ...
    def average_vertex(self):
        ...
        for i in range(num_shapes):
            vert_avg = vert_avg + 1 / num_shapes * self.shape_array[i].get_vert().clone()
    ...
    def forward(self):
        ...
        for i in range(len(self.shape_array)):
            E_curr = self.interpolation_coeff[i] * \
                     self.energy.forward_single(self.vert_new,
                                                self.shape_array[i].get_vert(),
                                                self.shape_array[i])
...
class InterpolationModuleSingleHessian(InterpolationModuleSingle, InterpolationModuleHessian):
    """Same as InterpolationModuleSingle, but with second order optimization"""

    def __init__(self, energy: InterpolationEnergyHessian, shape_array, interpolation_coeff):
...
class InterpolationModuleBvp(InterpolationModuleMultiscale):
    ...
    def __init__(self, energy: InterpolationEnergy, shape_x: Shape, shape_y: Shape, param=Param(), vertices=None):
        super().__init__(energy)
    ...
    def forward(self):
        ...
        E_x = self.energy.forward_single(self.vert_sequence[0, ...].detach(),
                                            self.vert_sequence[1, ...],
                                            self.shape_x)
        E_y = self.energy.forward_single(self.vert_sequence[num_t-2, ...],
        ...
        for i in range(1, num_t-2):
            E_curr = self.energy.forward_single(self.vert_sequence[i, ...],
                                                self.vert_sequence[i+1, ...],
                                                self.shape_x)
            E_total = E_total + E_curr
    ...
    def compute_vertices(self):
        ...
        for i in range(0, num_t):
            lambd = i / (num_t - 1)
            vertices[i, ...] = (1 - lambd) * self.shape_x.get_vert().clone() + \
                               lambd * self.shape_y.get_vert().clone()

        return vertices

    def copy_self(self, vertices=None):
        return InterpolationModuleBvp(self.energy, self.shape_x, self.shape_y, self.param, vertices)
    ...
    def upsample_resolution(self, vertices):
        ...
        for t in range(1, num_t-1):
            l = (t-1)/(num_t-2)
            vertices_new[t, ...] = (1-l) * self.shape_x.apply_upsampling(vertices[t, ...]) +\
    ...
    def insert_additional_vertices(self, vertices):
        ...
        vertices = vertices * torch.as_tensor([1, 0], device=device, dtype=torch.float32).unsqueeze(0).unsqueeze(2).unsqueeze(3)
        vertices = vertices.reshape([num_t*2, num_vert, 3])
        vertices = vertices[0:num_t*2-1, ...]

        for i in range(num_t-1):
            vertices[i*2+1, ...] = 0.5*(vertices[i*2, ...] + vertices[i*2+2, ...]).clone()
...
class InterpolationModuleBvpHessian(InterpolationModuleBvp, InterpolationModuleHessian):
    ...
    def copy_self(self, vertices=None):
        return InterpolationModuleBvpHessian(self.energy, self.shape_x, self.shape_y, self.param, vertices)
    ...
    def mul_with_inv_hessian(self):
        ...
        central_diff = lil_matrix(spdiags(central_diff_diags, np.array([-1, 0, 1]), num_t, num_t))
        ...
        hess = csr_matrix(kron(central_diff, hess_1d) + kron(boundary_cond, eye(n_vert)))

        grad_hess = spsolve(hess, self.vert_sequence.grad.view(-1, 3).to(device_cpu).detach().cpu())
        self.vert_sequence.grad = torch.tensor(grad_hess, dtype=torch.float32, device=device).view_as(self.vert_sequence)
    ...
    def interpolate(self, super_idx=-1):
        ...
        for it in range(self.param.num_it):
            ...
            if self.param.log:
                if self.param.log:
                    if super_idx >= 0:
                        print('Super {:02d}, It {:03d}, E: {:.5f}'.format(super_idx, it, Elist[0]))
                    else:
...
class InterpolationLBFGS(Interpolation):
    ...
    def interpolate(self, super_idx=-1):
        ...
        lr = self.param.lr
        optimizer = torch.optim.LBFGS(self.interp_module.parameters(),
...
if __name__ == "__main__":
    print("main of interpolation_base")
